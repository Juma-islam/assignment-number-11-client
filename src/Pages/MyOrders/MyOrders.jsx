                    ...
                    myOrders.map((order) => (
                        <div key={order._id} className="shadow rounded-lg p-4 border border-gray-100">
                            <div className="flex justify-between items-start mb-3">
                                ...
                                <span
                                    className={`px-2 py-1 rounded text-xs font-semibold whitespace-nowrap ${order.status === "Shipped"
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => navigate(`order-details/${order._id}`)} className="btn btn-primary hover:bg-primary/90 text-white rounded text-sm font-medium transition-colors flex-1">
                                    View Details
                                </button>
                                <button disabled={order?.status !== "Pending"} onClick={() => handleOrderDelete(order._id)} className="btn btn-error text-white rounded text-sm font-medium transition-colors flex-1">
                                    Cancel
                                </button>
                            </div>
                        </div>
                                ...
                                <tr key={order._id} className="border-b">
                                    ...
                                    <td className="p-3">
                                        <span
                                            className={`px-2 py-1 rounded text-xs font-semibold ${order?.status === "Pending"
                                                ? "bg-yellow-100 text-yellow-700" : order?.status === "Approved" ?
                                                    "bg-green-100 text-green-700" : order?.status === "Delivered" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                                                }`}
                                        >
                                            {order.status}
                                        </span>
                                    ...
                                    <td className="p-3">
                                        <div className="flex gap-2 justify-center">
                                            ...
                                            <button disabled={order?.status !== "Pending"} onClick={() => handleOrderDelete(order._id)} className="btn btn-error text-white rounded text-sm font-medium transition-colors">
                                                Cancel
                                            </button>
                                        </div>
                                    </td>
